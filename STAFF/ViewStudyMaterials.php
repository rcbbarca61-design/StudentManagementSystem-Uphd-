...
$a = "SELECT `studymaterials`.*,`staff`.*,`department`.* FROM `department`,`studymaterials`,`staff` WHERE `studymaterials`.`sid`=`staff`.`sid` AND `department`.`deptname`=`studymaterials`.`stmdept` AND `staff`.`sid`='$uid'";
$qry = mysqli_query($conn, $a);
if (mysqli_num_rows($qry) < 1) {
?>
    <center>
        <h1 style="color: black;">No StudyMaterials Added</h1>
    </center>
<?php
} else {
?>
