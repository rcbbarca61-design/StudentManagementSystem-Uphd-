<!DOCTYPE html>
	<html lang="zxx" class="no-js">
	<head>
		<!-- Mobile Specific Meta -->
		<meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
		<!-- Favicon-->
		<link rel="shortcut icon" href="../img/book.png">
		<!-- Author Meta -->
		<meta name="author" content="colorlib">
		<!-- Meta Description -->
		<meta name="description" content="">
		<!-- Meta Keyword -->
		<meta name="keywords" content="">
		<!-- meta character set -->
		<meta charset="UTF-8">
		<!-- Site Title -->
		<title>Easy Edu</title>
		<!-- <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css"> -->

		<link href="https://fonts.googleapis.com/css?family=Poppins:100,200,400,300,500,600,700" rel="stylesheet"> 
			<!--
			CSS
			============================================= -->
			<link rel="stylesheet" href="../css/linearicons.css">
			<link rel="stylesheet" href="../css/font-awesome.min.css">
			<link rel="stylesheet" href="../css/bootstrap.css">
			<link rel="stylesheet" href="../css/magnific-popup.css">
			<link rel="stylesheet" href="../css/nice-select.css">							
			<link rel="stylesheet" href="../css/animate.min.css">
			<link rel="stylesheet" href="../css/owl.carousel.css">			
			<link rel="stylesheet" href="../css/jquery-ui.css">			
			<link rel="stylesheet" href="../css/main.css">
		</head>
		<body>	
		  <header id="header" id="home">
	  		<div class="header-top">
	  			<div class="container">
			  		<div class="row">
			  			<div class="col-lg-6 col-sm-6 col-8 header-top-left no-padding">
			  				<ul>
								<li><a href="#"><i class="fa fa-facebook"></i></a></li>
								<li><a href="#"><i class="fa fa-twitter"></i></a></li>
								<li><a href="#"><i class="fa fa-dribbble"></i></a></li>
								<li><a href="#"><i class="fa fa-behance"></i></a></li>
			  				</ul>			
			  			</div>
			  			<div class="col-lg-6 col-sm-6 col-4 header-top-right no-padding">
			  				<a href="tel:+953 012 3654 896"><span class="lnr lnr-phone-handset"></span> <span class="text">+953 012 3654 896</span></a>
			  				<a href="mailto:support@colorlib.com"><span class="lnr lnr-envelope"></span> <span class="text">support@colorlib.com</span></a>			
			  			</div>
			  		</div>			  					
	  			</div>
			</div>
		    <div class="container main-menu">
		    	<div class="row align-items-center justify-content-between d-flex">
			      <div id="logo">
			        <a href="index.php"><img src="../img/logo.png" alt="" title="" /></a>
			      </div>
			      <nav id="nav-menu-container">
			        <ul class="nav-menu">
                    <li><a href="StaffHome.php">Home</a></li>
			          <!-- <li><a href="../about.php">About</a></li> -->	
					  <li class="menu-has-children"><a href="#">Timetable</a>
			            <ul>
							<li><a href="AddTimetable.php">Add Timetable</a></li>
							<li><a href="ViewTimetable.php">View Timetable</a></li>
			            </ul>
			          </li>	
					  <li class="menu-has-children"><a href="#">StudyMaterials</a>
			            <ul>
							<li><a href="AddStudyMaterials.php">Add StudyMaterials</a></li>
							<li><a href="ViewStudyMaterials.php">View StudyMaterials</a></li>
			            </ul>
			          </li>
					  <li class="menu-has-children"><a href="#">Event</a>
			            <ul>
							<li><a href="AddEvent.php">Add Event</a></li>
							<li><a href="ViewEvent.php">View Event</a></li>
			            </ul>
			          </li>	
					  <li class="menu-has-children"><a href="#">View</a>
			            <ul>
							<li><a href="ViewStudents.php">Students List</a></li>
							<li><a href="ViewCounselling.php">Counselling List</a></li>
			            </ul>
			          </li>
			          <li><a href="ChangePassword.php">Change Password</a></li>
			          <li><a href="SendFeedback.php">Send Feedback</a></li>
					  <li class="menu-has-children"><a href="#">Complaint</a>
			            <ul>
							<li><a href="SendComplaint.php">Send Complaint</a></li>
							<li><a href="ViewComplaintRep.php">View Reply</a></li>
			            </ul>
			          </li>	
			          <li><a href="../index.php">LogOut</a></li>
			        </ul>
			      </nav><!-- #nav-menu-container -->		    		
		    	</div>
		    </div>
		  </header><!-- #header -->