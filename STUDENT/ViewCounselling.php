...
$a = "SELECT `student`.`studid`,`fname`,`counsellor`.`co_name`,`co_id`,`chat_members`.* FROM `chat_members`,`student`,`counsellor` WHERE `counsellor`.`co_id`=`chat_members`.`c_id` AND `student`.`studid`=`chat_members`.`studid` AND `student`.`studid`='$uid'";
                ...
                <table id="table" class="table table-bordered table-striped mb-5">
                    ...
                    <tbody id="tableBody">
                        ...
                        while ($row = mysqli_fetch_array($qry)) {
                            $counsellor_id = $row['co_id']; 
                        ?>
                            <tr class="text-center" style="color: black;">
                                <td><?php echo $row['co_name']; ?></td>
                                <!-- <td><?php echo $row['fname']; ?></td> -->
                                <td><div class="d-flex justify-content-center align-items-center">
                                        <a href="StudentChat.php?id=<?php echo $row['chat_mid'] ?>&counsellor_id=<?php echo $counsellor_id; ?>" class="btn btn-outline-danger ml-2">Chat</a>
                                    </div></td>
                            </tr>
                        <?php
                        }
